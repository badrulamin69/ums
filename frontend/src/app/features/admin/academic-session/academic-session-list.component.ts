...
export class AcademicSessionListComponent implements OnInit {
  ...
  loadPage(page: number): void {
    ...
    this.crud.list<Session>('academic-sessions', page, 10).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
  ...
  doDelete(): void {
    ...
    if (!item) return;
    this.crud.delete('academic-sessions', item.id).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
